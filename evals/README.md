...
量化"领域知识检索"的召回质量。知识源是 22 篇长文档（`evals/fixtures/knowledge_documents.py`，18 篇风格 + 4 篇综合指南），入库时用可配置的 `chunk_size`/`chunk_overlap` 切分（`evals/build_knowledge_store.py`）。

### 数据集

`evals/datasets/rag_retrieval_seed.jsonl` 共 58 条（风格 20 / 材料 10 / 植物 8 / 施工 7 / 布局 5 / 组合 5 / 负样本 3）。相关性**标注在文档层**（`relevant_docs` 是文档 id），换切分参数后依然可比：

```json
{"id": 1, "question": "新中式庭院有什么特点", "relevant_docs": ["new-chinese"], "category": "style"}
```

### 使用方法

```bash
uv run python evals/build_rag_seed_dataset.py      # 生成/更新数据集
uv run python evals/build_knowledge_store.py       # 切分入库（--chunk-size/--chunk-overlap 可配）
uv run python evals/run_rag_eval.py --show-misses  # 评测
```

### 当前基线（200/30 切分，vector + 距离阈值 1.30，55 条非负样本）
...
| 负样本 top-1 误命中率 | **0.0%**（越低越好） |

### 任务 6 结论（混合检索与切分对比）

- **距离阈值门控**（`--max-distance`）：数据驱动分析正样本 top-1 距离最大 1.22、负样本最小 1.45，取阈值 1.30——**召回完全不变，负样本误命中从 100% 降到 0%**；
- **混合检索（向量 + BM25 + RRF）**：在当前 27 chunk 的小语料上反而略降（recall@5 96.7%→95.2%，负样本仍有残留），结论是**小语料上纯向量 + 阈值已是最优**，BM25 融合是噪声；
- **切分参数对比**（每组合自动重调阈值）：

| chunk_size | overlap | recall@1 | recall@5 | MRR | 负样本误命中 |
| --- | --- | --- | --- | --- | --- |
| 100 | 20 | 73.0% | 94.8% | 0.955 | 0.0% |
| **200** | **30** | **74.9%** | **96.7%** | **0.961** | 0.0% |
| 300 | 50 | 73.0% | 94.8% | 0.950 | 0.0% |

结论：当前知识规模下 **200/30 切分 + 向量检索 + 阈值 1.30** 为最优配置；若未来语料变大、术语查询变多，再重测混合检索与 rerank。

## RAG 回答评测（LLM-as-judge）

`evals/run_rag_answer_eval.py` 对知识咨询问题分别生成"无 RAG / 有 RAG"回答，用 DeepSeek 评审（temperature=0）按正确性 / 有据率 / 无幻觉 / 覆盖度 / 简洁性 打分，量化 RAG 的价值。

```bash
uv run python evals/run_rag_answer_eval.py --limit 20   # 冒烟/基线
uv run python evals/run_rag_answer_eval.py              # 全量（55 条，约 15 分钟）
```
...
| 维度 | 无 RAG | 有 RAG | 差值 |
| --- | --- | --- | --- |
| 正确性 | 0.870 | 0.980 | +0.110 |
| 有据率 | 0.600 | 0.980 | **+0.380** |
| 无幻觉 | 0.840 | 0.970 | +0.130 |
| 覆盖度 | 0.900 | 0.980 | +0.080 |
| 简洁性 | 0.640 | 0.780 | +0.140 |

judge 通过率 90% → 100%，平均分 0.770 → 0.938，RAG 幻觉率 0.0%。**结论：RAG 显著提升回答质量，有据率是最大收益点（+38 个百分点）**。
