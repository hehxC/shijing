...
def _build_prompt(message: str, has_reference_image: bool) -> str:
    ...
    if "第 1 张输入图是庭院空间图" in user_request:
        reference_rule = (
            "第 1 张输入图是庭院空间图，只用于约束真实场地条件。"
            "必须保持原图的相机位置、观察方向、透视关系、庭院范围、建筑主体体量、"
            "主要永久墙体边界、出入口和基本高差，使用户能够明确认出这是同一个庭院；"
            "不得凭空扩大场地、改变建筑层数或移动不可拆除的主体结构。"
            "除上述不可变条件外，必须把画面理解为一次完整改造，而不是在旧照片上简单添加装饰。"
            "原有墙地面颜色与饰面、破损污渍、裸露水泥、杂物、晾衣绳、裸露电线、"
            "废弃管道、旧花盆、临时棚架和其他非永久设施都不属于需要保留的空间结构，"
            "应清除、修复、遮挡或重新设计。"
            "允许全面更新墙面、地面、非结构性门窗外观、排水、照明、植物和景观设施，"
            "不要继承原图破旧、杂乱、拥挤、阴暗或廉价的视觉气质。"
            "优化空间秩序、纵深层次和视觉焦点，控制景观元素数量，避免堆砌假山、灯具、"
            "花池或植物；整体材料与色彩应统一、克制、高级，并符合真实尺度和施工逻辑。"
        )
    elif "第 1 张输入图是上一轮已生成的效果图" in user_request:
        reference_rule = (
            "第 1 张输入图是上一轮已生成的效果图。保持其相机视角、构图、透视、"
            "建筑边界和所有未被用户要求修改的设计，只准确执行本轮修改。"
        )
    elif has_reference_image:
        reference_rule = (
            "输入图片仅作为材料颜色、纹理和质感参考，不代表庭院空间布局。"
            "请根据用户要求自行完成完整、合理且具有设计感的庭院空间设计。"
        )
    else:
        reference_rule = "根据用户的文字描述完成完整、合理且具有设计感的庭院空间设计。"
    return (
        f"{reference_rule}\n"
        f"用户需求：{user_request}\n"
        "最终画面应像专业景观设计事务所完成的高品质建成摄影：风格明确，构图完整，"
        "空间尺度合理，材料纹理真实，植物疏密有致，光线明亮柔和，施工逻辑可落地。"
        "只生成一张照片级真实的最终效果图，不生成前后对比图，不添加水印、标题、标注或无关文字。"
    )
