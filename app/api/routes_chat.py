...
def guest_session_id(session_id: str | None) -> str:
    raw = session_id or "default"
    value = f"guest:{raw}"
    if len(value) <= 128:
        return value
    return f"guest:{hashlib.sha256(raw.encode('utf-8')).hexdigest()}"
...
@router.post("/chat")
async def chat(req: ChatRequest, current_user: User | None = Depends(get_optional_current_user)):
    """
    聊天接口：接收用户消息，返回模型回复的流式响应。

    请求体 JSON 格式:
    - 纯文本: {"message": "你好"}
    - 图文: {"message": "分析这张图", "image": "data:image/jpeg;base64,..."}
    - 可选会话: {"message": "你好", "session_id": "browser-session-id"}
    响应: text/plain 类型的流式数据，逐 token 返回
    """
    session_id = (
        scoped_session_id(current_user.id, req.session_id)
        if current_user
        else guest_session_id(req.session_id)
    )
